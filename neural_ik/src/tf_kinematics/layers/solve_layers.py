...
class SolveCompactIterGrad(_KinematicLayer):
    ...
    def call(self, inputs, *args, **kwargs):
        y_goal, x = inputs
        return iter_grad(y_goal, x, self.__sys_fn, self.__loss)
...
@tf.keras.utils.register_keras_serializable()
class SolveCompactIterGradHess(_KinematicLayer):
    def __init__(self, loss_ident, kin_model_name: str, batch_size: int, *args, **kwargs):
        self.__loss = None
        self.__sys_fn = None
        self.loss_ident = loss_ident
    ...
    def get_config(self):
        config = super(SolveCompactIterGradHess, self).get_config()
        config.update({'loss_ident': self.loss_ident})
        return config
