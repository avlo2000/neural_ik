...
class Test(TestCase):
    def test_residual_acos_fk_dnn_forward(self):
        batch_size = 2
        blocks_count = 2
        kin = load_kin("kuka_robot", batch_size)
        model_dist, model_ik = residual_acos_fk_dnn("kuka_robot", batch_size,
    ...
    def test_residual_acos_fk_dnn_save(self):
        batch_size = 2
        blocks_count = 2
        model_dist, model_ik = residual_acos_fk_dnn("omnipointer_robot", batch_size,
                                                    blocks_count=blocks_count, corrector_units=2)
